...
- `recurring.service.ts`/`import.service.ts` ไม่ผ่าน `TransactionService` เลย (เขียน side-effect เอง) — **ไม่มี budget-insufficient check, ไม่ lock แถว, และตอนนี้ข้าม closed-period guard ด้วย** (guard อยู่ใน `transaction.service.ts`/`pool.service.ts` เท่านั้น) เป็น gap เดิมตั้งแต่ก่อน Borrow feature ไม่ใช่ regression ใหม่ แต่ **ความเสี่ยงสูงขึ้นหลัง Phase 3** เพราะตอนนี้มี "เดือนปิดแล้ว" ที่ต้อง freeze จริงๆ ให้ recurring/import เขียนเข้าเดือนปิดได้เงียบๆ จะ corrupt `BudgetMonthlyHistory` ที่ freeze ไปแล้วโดยไม่มีใครรู้ — พึ่ง `notifyBudgetAlerts` แจ้งย้อนหลังแทน ยังไม่ตัดสินใจว่าจะให้ borrow + closed-period guard เข้าถึง path นี้ด้วยไหม
  - **ญาติของ gap เดียวกัน เจอเพิ่มระหว่างทำ month switcher (2026-09-17)**: `assertBudgetsPeriodOpen` เรียกเฉพาะตอน `dto.budgetId && dto.type === 'EXPENSE'` (`transaction.service.ts` `create()`) — **INCOME transaction ไม่เคยผ่าน guard นี้เลย ไม่ว่ากรณีไหน** เพราะ INCOME ไม่ผูก budget ตามดีไซน์ ผลคือ user บันทึก INCOME ย้อนหลังวันไหนก็ได้ตลอดไป ไม่มี guard กันเลย ต่างจาก recurring/import ตรงที่นี่ **ไม่ใช่ gap ที่ตั้งใจข้าม แต่เป็นผลข้างเคียงของเงื่อนไข guard เอง** — ยังไม่ตัดสินใจว่าจะแก้ยังไง (INCOME ควรมี guard แบบไหนในเมื่อไม่มี budget ให้ผูก — อาจต้องคิดเกณฑ์ "เดือนปิด" แยกสำหรับ INCOME โดยเฉพาะที่ไม่อิง budget session) — เจอจริงกับ demo user จาก transaction มิถุนายน 2026 ที่ไม่มี guard ตอนสร้าง (แต่ในเคส demo ต้นเหตุจริงคือ budget สร้างก่อน migration ที่เพิ่ม periodYear/periodMonth วันที่ 2026-08-16 ทำให้ dbgenerated default ตราวันที่ migration ไม่ใช่วันที่สร้างจริง — เป็น one-time migration artifact เฉพาะบัญชีเก่า ไม่ใช่สาเหตุเดียวกับ gap นี้ แต่ gap นี้จะเกิดกับ user ใหม่ตลอดไปถ้าไม่แก้) — แก้ `hasSessionBefore()`/month-switcher's `hasPrevious` ให้เช็ค `Transaction` จริงคู่กับ `BudgetSession` ไปแล้ว (ไม่ใช่การแก้ root cause แต่กันไม่ให้ month switcher บล็อกข้อมูลจริงที่มีอยู่)
...
### Phase A (mirror-write) + Month Switcher — เสร็จแล้ว, Phase B ยังค้างอยู่ตั้งใจ (2026-09-17)

Design ใหม่แทนที่แผน "implementation phase เต็ม ~10 call site" ด้านบน — พบว่าไม่จำเป็นต้องทำ full cutover เพื่อได้ month switcher ตามที่ user ขอ ดู plan doc คู่: `C:\Users\ammar\.claude\plans\kind-dancing-sparrow.md` (Phase A) + `C:\Users\ammar\.claude\plans\linear-honking-hamming.md` (month switcher)

- **Phase A (mirror-write, เสร็จ)**: `mirrorSessionAmount()` (`backend/src/utils/budget-session.ts`) เขียนคู่ไปกับทุกจุดที่แก้ `Budget.spentAmount`/`allocatedAmount` เดิม (16 delta-mirror call site + 2 lifecycle site — `budget.repository.ts create()`, `commitMonthClose()`) ทำให้ `BudgetSession` เป็นสำเนาที่ถูกต้องเสมอของ `Budget` field เดิม โดย**ไม่มี read ไหนอ่าน `BudgetSession` เลย** — live path (`addStats`/`getAllocationTotals`/invariant check ทุกจุด) ยังอ่าน `Budget` เหมือนเดิม 100% ความเสี่ยงต่ำมาก **เป็นพฤติกรรมถาวร ไม่ใช่ migration ครั้งเดียว** — ต้องรันต่อไปเรื่อยๆ เพราะเดือนที่จะปิดในอนาคตต้องมี `BudgetSession` รอให้ month switcher อ่านได้ทันที
- **Month Switcher (Dashboard + Budgets, เสร็จ)**: เดือนย้อนหลังเป็น read-only อยู่แล้ว (closed-period guard บล็อกการเขียนอยู่แล้ว) เลยสร้าง read ใหม่ต่างหากที่อ่าน `BudgetSession` ตรงๆ สำหรับ period ที่ไม่ใช่ปัจจุบัน (`BudgetService.getForPeriod`, `DashboardService.getSummaryForPeriod` — เมธอดใหม่ ไม่ได้แทนที่ `getAll()`/`getSummary()` เดิมที่ `pool.service.ts`/`reports.service.ts`/`household.service.ts` ยังเรียกอยู่) โดย**ไม่ต้องแตะ live path ที่เสี่ยงเรื่อง invariant เลย** response shape เหมือนกันเป๊ะไม่ว่าจะดูเดือนปัจจุบันหรือย้อนหลัง (frontend ไม่มี if/else แยกโค้ดสองทาง — branch อยู่ฝั่ง backend ล้วน)
- **Phase B (full read cutover: `addStats`/`getAllocationTotals`/`dashboard.service.ts`/`reports.service.ts`/`period-guard.ts` ไปอ่าน `BudgetSession` แทน `Budget` field เดิม) ยังไม่ทำ — ตั้งใจเลื่อนออกไป ไม่ได้ยกเลิก** เหตุผล: month switcher (use case จริงที่ต้องการตอนนี้) ไม่จำเป็นต้องมี Phase B เลย เพราะเป็น read-only อย่างเดียว การทำ full cutover ตอนนี้จะเพิ่มความเสี่ยงต่อ invariant-critical code โดยไม่ได้ประโยชน์เพิ่ม — **ความเสี่ยง**: แผนแบบ "additive ไปก่อน ทำ cutover เสี่ยงทีหลัง" แบบนี้เคยถูกลืมมาแล้วครั้งหนึ่งในโปรเจกต์นี้ (cron daily job ไม่เคยถูก wire เข้า production, `9330fa0`) **ตัวกระตุ้นให้กลับมาทำ Phase B**: ครั้งหน้าที่มีเหตุผลอื่น (ไม่เกี่ยวกับ month switcher) ต้องแก้ไฟล์ที่อ่าน `Budget.spentAmount`/`allocatedAmount` ตรงๆ อยู่แล้ว ให้ทำ cutover เฉพาะจุดนั้นไปพร้อมกันเลย แทนที่จะแก้ column เดิมต่อไป — ดู punch list เต็มใน `merry-bubbling-rabbit.md` (ยังใช้ได้ ไม่มีอะไรเปลี่ยน)

**Test run จริงแล้ว (2026-09-17, local Docker/`budgetflow_test`)**: backend `npm run test` 76/76 ผ่าน (14 test file รวม `budget.period-read.test.ts` และ `dashboard.period-read.test.ts` ที่เขียนใหม่ทดสอบ `getForPeriod`/`getSummaryForPeriod` — current-period ตรงกับ path เดิมเป๊ะ, future period ถูก reject 400, period ที่ไม่มี session เลยได้ array ว่าง, เดือนปิดแล้วอ่านตัวเลขแช่แข็งถูกต้องรวม budget ที่ archive ไปแล้ว, `hasPrevious` พลิกถูกที่ขอบเขต) + `npm run lint`/`npm run build` backend และ frontend ผ่านทั้งคู่

**บั๊กที่เจอระหว่างเขียน test ใหม่ (แก้แล้วก่อน commit)**: `commitMonthClose()` เดิม (ตอนเขียน Phase A) stamp แค่ `allocatedAmount`/`spentAmount` ตอนปิด session ไม่ได้ stamp `periodYear`/`periodMonth`/`rolloverPolicy` ด้วย — ไม่กระทบ production จริง (Budget.periodYear เปลี่ยนได้แค่ผ่าน `commitMonthClose()` เองซึ่ง sync กับ session อยู่แล้วโดยธรรมชาติ) แต่ `rolloverPolicy` เปลี่ยนได้จริงผ่าน `BudgetService.update()` กลางเดือนโดยไม่เคย mirror เข้า session เลย ทำให้ historical read เจอ policy ผิด (เทสจับได้จริงตอนตั้ง fixture) — แก้โดย (1) เพิ่ม `mirrorSessionRolloverPolicy()` ใน `budget-session.ts` เรียกจาก `update()` เมื่อ `dto.rolloverPolicy` เปลี่ยน (2) ให้ `commitMonthClose()` stamp `periodYear`/`periodMonth`/`rolloverPolicy` ตรงๆ จาก live `Budget` row ตอนปิดเสมอ (ตาม philosophy เดิมของโค้ดบรรทัดนั้นที่ทำกับ `allocatedAmount`/`spentAmount` อยู่แล้ว) — ทั้งสองจุดยังไม่เคย commit มาก่อน แก้ในรอบเดียวกันนี้เลย ไม่ใช่ bug ที่เคยขึ้น production

**Manual browser pass ผ่าน local dev server จริงแล้ว (2026-09-17)** — login demo user, ทดสอบทั้ง Dashboard และ Budgets:
- **บั๊กจริงที่เจอและแก้แล้ว**: ปุ่ม "ถัดไป" (next) บน `MonthSwitcher` กลับตรรกะ — โค้ดเดิมเขียน `hasNext={!isPastMonth}` (เปิดปุ่มตอนอยู่เดือนปัจจุบัน ปิดปุ่มตอนอยู่เดือนย้อนหลัง) ตรงข้ามกับที่ควรเป็น (ต้องปิดตอนอยู่เดือนปัจจุบันกันไปอนาคต เปิดตอนอยู่เดือนย้อนหลังให้กลับมาปัจจุบันได้) — จับได้จริงตอนกด next จากเดือนสิงหาคมแล้วกลับเดือนกันยายนไม่ได้ (ปุ่มไม่ทำงานเงียบๆ ไม่ error ไม่ crash) แก้เป็น `hasNext={isPastMonth}` ทั้ง `DashboardPage.tsx` และ `BudgetsPage.tsx` (โค้ดชุดเดียวกัน copy กันมา) ยืนยันซ้ำหลังแก้ว่า prev→August→next→September ทำงานถูกต้อง รวมกดปุ่ม next ตอนอยู่เดือนปัจจุบันแล้วไม่ยิง request เลย (verified ผ่าน network tab)
- Dashboard: banner ถาวรขึ้น/หายถูกจังหวะ, ปุ่ม Allocate Income disabled จริง (กดไม่เปิด modal), ตัวเลขเปลี่ยนตามเดือนถูกต้อง (เทียบกับ query DB ตรงๆ), Budget Alerts banner หายไปเองเพราะ `alerts: []` ตามดีไซน์
- Budgets: banner, ปุ่ม Add Budget/Allocate Income disabled จริง (กดไม่เปิด modal), การ์ด quick-add หายไปตอนเดือนย้อนหลัง, `BudgetCard` เมนู edit/delete หายจริงจาก DOM ทั้งก้อน (ไม่ใช่แค่ซ่อนด้วย CSS — เช็คด้วย `document.querySelectorAll('button[aria-label="เมนู"]').length === 0`)
- Network tab: query param year/month เปลี่ยนตามจริงทุกครั้ง ไม่มี cache ปนข้ามเดือน (เช็คด้วย `read_network_requests` ตรงๆ)
- เดือนที่ไม่มี session เลย: สมัคร user ใหม่ล้วนๆ (0 budget ทุกตัว) เข้าทั้ง Dashboard และ Budgets ไม่ crash, ไม่มี console error, ปุ่ม prev ถูก disable ถูกต้อง (ไม่มี session ให้ย้อนกลับ)
- **หมายเหตุด้าน tooling**: ระหว่างเทสเจอปุ่ม prev/next "กดไม่ติด" หลายครั้งที่กลับกลายเป็น browser-automation click ไม่ลงจริง (ref เก่าค้าง/พิกัดไม่ตรง) ไม่ใช่บั๊กแอป — วิธียืนยันคือเช็ค `button.disabled` ผ่าน JS ตรงๆ + ลอง `.click()` ผ่าน `javascript_tool` เทียบกับผลจาก `computer` tool
- **มือถือ viewport จริง (390px) — ลองแล้วแต่ยืนยันด้วยภาพไม่ได้**: `resize_window` รายงานว่าสำเร็จ (390x844) แต่ `window.innerWidth` ที่อ่านจริงในหน้ายังเป็น 1536 เท่าเดิม — เครื่องมือ resize ไม่ mirror ไปที่ viewport จริงในเครื่องนี้ ปัญหาเดิมที่เคยเจอกับ `BorrowBudgetModal` ตอน Phase 2 (บันทึกไว้ใน CLAUDE.md แล้ว) ไม่ใช่ปัญหาใหม่ — ไม่ไล่ตามต่อ (จะกลายเป็น rabbit hole เรื่อง tooling ไม่ใช่เรื่องโค้ด) แทนที่ด้วยการเช็ค markup/class ตรงๆ: ยืนยันแล้วว่า header ทั้งสองหน้า (`BudgetsPage.tsx`/`DashboardPage.tsx`) มี `flex-wrap` ครอบกลุ่ม switcher+ปุ่ม, banner ไม่มี `hidden`/`md:` class ใดๆ (แสดงเต็มความกว้างทุก viewport, ไม่ผูกกับ hover ตามที่ตกลงไว้แล้ว), `MonthSwitcher` label กว้างแค่ `min-w-[9rem]` ไม่เสี่ยง overflow บน 390px — **แต่ยังไม่เคยเห็นด้วยตาบนมือถือจริงหรือ viewport แคบจริง** เหมือนที่ `BorrowBudgetModal`/split-detail line ก็ยังค้างข้อเดียวกันอยู่ (ดูหัวข้อ Phase 2 ด้านบน) — รวมเป็น known gap เดียวกัน ยังไม่ปิด

**มือถือ viewport จริง (390px) — ทดสอบแล้วผ่าน (2026-09-17 ต่อมาในวันเดียวกัน)**: user เทสเองบนโทรศัพท์จริงผ่าน LAN (`vite.config.ts`'s `server.host: true` มีอยู่แล้ว ใช้ IP เครื่อง `192.168.56.125:5174` แทน Vercel preview — เร็วกว่าและเลี่ยง `budgetflow_vercel_preview_url_trap` ไปในตัว) banner อ่านออกไม่ล้นจอ, ปุ่ม disabled กดไม่ติดจริง, switcher กดง่าย — commit แล้ว push แล้ว: `7b4f0fc` (Phase A) + `803a9ba` (month switcher Dashboard/Budgets)

**Round 2 (2026-09-17 เช่นกัน) — ผูก month switcher ให้ครบ 2 จุดที่ยังไม่ผูก + ปิดช่องโหว่ที่เจอระหว่างทาง**:
1. **Dashboard Recent Transactions** — เจอ production จริงว่าไม่กรองตามเดือนที่เลือกดู root cause: `getSummaryForPeriod()`'s `isCurrent` branch เอา `getSummary()` มาทั้งดุ้น ซึ่ง `getSummary()`'s recentTransactions ไม่เคยกรองเดือนเลยตั้งแต่ก่อนมี month switcher (`where:{userId}` ไม่มี date bound) — แก้โดย extract `getRecentTransactionsForMonth()` ใช้ร่วมทั้งสอง branch, **ไม่แตะ `getSummary()` เอง** (household.service.ts ยังใช้ unbounded ตามเดิม ตั้งใจ)
2. **Transactions page** — เพิ่ม month switcher + ปุ่ม "ดูทั้งหมด" ติดข้าง switcher ตลอด (ไม่ซ่อนในเมนู) default เดือนปัจจุบัน toggle ไม่ reset search/type/budget filter เดิม — backend reuse `startDate`/`endDate` filter ที่มีอยู่แล้ว เพิ่มแค่ controller แปลง `year`/`month` ผ่าน `bangkokMonthRangeUtc` (ไม่แตะ repository/service) **ไม่มี lower bound แบบ Dashboard/Budgets** (ตั้งใจ — เดือนอนาคตแค่ list ว่างไม่มีความเสี่ยงข้อมูลเพี้ยนแบบ BudgetSession)
3. **`hasSessionBefore()` เช็คแค่ `BudgetSession` ไม่พอ** — user ทักท้วงหลังเจอ Dashboard บล็อกไม่ให้ย้อนไปมิถุนายนทั้งที่มี transaction จริง ตรวจแล้วเป็นช่องโหว่จริงที่กระทบ user ทั่วไปได้ (ไม่ใช่แค่ demo): INCOME ไม่เคยผ่าน `assertBudgetsPeriodOpen` เลย (ดู bullet ใหม่ด้านบนในหมวด known gaps) + CSV import ข้าม guard เดิมอยู่แล้ว ทั้งสองทำให้ user มี transaction เก่ากว่า BudgetSession แรกได้จริง แก้ `hasSessionBefore()` ให้เช็คทั้ง `BudgetSession` และ `Transaction` (OR กัน)

**Test**: backend `npm run test` 79/79 ผ่าน (เพิ่ม test ใหม่ 3 เคส: Dashboard bound เดือนปัจจุบันถูก, transaction filter ตาม year/month ถูก, `hasPrevious` เป็น true จาก Transaction อย่างเดียวแม้ไม่มี session เลย) `lint`/`build` ผ่านทั้งคู่ ยืนยันสดผ่าน API จริงกับ demo user ว่า `/budgets?year=2026&month=6` เข้าได้แล้วและ `/dashboard?year=2026&month=6` โชว์ transaction มิถุนายนครบ — commit + push แล้ว
